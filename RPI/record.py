...
def detect_micturition(frames):
        ...
        sounds = audio_classification.yamnet_run_inference(data0)
        print(sounds)

        # compare known valid water sounds to yamnet's 3 most likely sound sources
        # If sound is water -> then run our void-detection model
        for sound in sounds:
            if "Water" in sound or "Drip" in sound or "Liquid" in sound or "splatter" in sound:
                ...
                micturition = runInference.detect_micturition(wav_data)
                print(micturition)
                if micturition:
